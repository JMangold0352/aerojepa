...
Prefer `attitude_rmse_geodesic_deg` / `geodesic_attitude_error_deg` in
...
**Note:** Headline AeroProber position RMSE tables (~0.006 m at 0.1 s) were
...
- Do not compare AeroProber ~0.006 m (0.1 s sim) to SkyJEPA outdoor open-loop RMSE.
...
- Action-conditioned Wilds does **not** yet pass true/zero/shuffle counterfactuals
  (`results/action_counterfactual.json`); do not call it a causal world model.
